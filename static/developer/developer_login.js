const login = async() => {
    ...
    const search_url = `/api-developer-login/?username1=${username}&password1=${password}`;
    ...
    try {
        ...
        if (responce.status === 200){
            ...
            document.querySelector('#alert').innerHTML = `No ${username} username in the Database or Incorrect Password`;
            document.querySelector('#alert').classList.remove('alert-success');
            document.querySelector('#alert').classList.add('alert-danger');
        }else{
            document.querySelector('#alert').innerHTML = 'Incorrect Password';
